...
- startData(): A function to return the default save data for this layer. Add any variables you have to it. Make sure to use `Decimal` values rather than normal numbers. Things added here will be in the player object.
  
```
added automatically (you don't need to add these but you can always use these):
    - unlocked: a bool determining if this layer is unlocked or not
    - points: a Decimal, the main currency for the layer
    - best: a Decimal, trcks best amount of points for this layer
    - total: a Decimal, tracks total amount of points for this layer
    - resets: a Decimal, tracks amount of resets done on this layer
    - bestOnReset: a Decimal, tracks best amount of points gained in one reset for this layer
    - resetTime: a number, tracks time since last reset for this layer
Optional (undefined === false):
    - ShowBest: a boolean, determines if it should display best points for this layer
    - showTotal: a boolean, determines if it should display 
    - unlockOrder: used to keep track of relevant layers unlocked before this one.
    - showResetTime: a boolean, determines if it should display time since last prestige for this layer 
    - showResets: a boolean, determines if it should display amount of resets done on this layer
    - showBestOnReset: a boolean, determines if it should display best amount of points gained in one reset for this layer
```
...
--resstsGainMult(): **optional**. multiplies amount of resets gained each prestige. defaults to 1 if not used.
