...
function loadVue() {
  ...
  Vue.component("display-text", {
    props: ["layer", "data"],
    template: `<component :is="{ template: '<span class=instant>' + data + '</span>'}"/>`
  });

  // Blank space, data = object with width and height
  Vue.component("blank", {
    props: ["layer", "data"],
    template: `
			<div class="instant">
			<div class="instant" v-bind:style="{width: (data?.width ? data.width : '8px'), height: (data?.height ? data.height: '17px')}"></div>
			</div>
		`,
  });
  ...
  Vue.component("display-image", {
    props: ["layer", "data"],
    template: `
			<img class="instant" v-bind:src="data" v-bind:alt="'can't load the image for some reason ' + data">
		`,
  });
  ...
  Vue.component("row", {
    ...
    template: `
        ...
        <div v-if="isPlainObject(item) v-bind:is="item.nane" :layer="item.layer ? item.layer : layer" v-bind:style="[[tmp[layer].componentStyles[item.name]], (item.style ? item.style : {})]" :data="item.data" :key="key + '-' + index"></div>
    ...
    template: `
		<div class="upgTable instant">
			<div class="upgCol">
				<div v-for="(item, index) in data">
					<div v-if="!Array.isArray(item)" v-bind:is="item" :layer= "layer" v-bind:style="tmp[layer].componentStyles[item]" :key="key + '-' + index"></div>
          <div v-if="isPlainObject(item)" v-bind:is="item.name" :layer="item.layer ? item.layer : layer" v-bind:style="[tmp[layer].componentStyles[item.name], (item.style ? item.style : {})]" :data="item.data" :key="key + '-' + index"></div>
					<div v-else-if="item.length==3" v-bind:style="[tmp[layer].componentStyles[item[0]], (item[2] ? item[2] : {})]" v-bind:is="item[0]" :layer= "layer" :data= "item[1]" :key="key + '-' + index"></div>
					<div v-else-if="item.length==2" v-bind:is="item[0]" :layer= "layer" :data= "item[1]" v-bind:style="tmp[layer].componentStyles[item[0]]" :key="key + '-' + index"></div>
				</div>
			</div>
    ...
    template: `
		<div class="story instant" v-if="tmp[layer].infoboxes && tmp[layer].infoboxes[data]!== undefined && tmp[layer].infoboxes[data].unlocked" v-bind:style="[{'border-color': tmp[layer].color, 'border-radius': player.infoboxes[layer][data] ? 0 : '8px'}, tmp[layer].infoboxes[data].style]">
			<button class="story-title" v-bind:style="[{'background-color': tmp[layer].color}, tmp[layer].infoboxes[data].titleStyle]"
				...
				<display-text :data="tmp[layer].infoboxes[data].title"></display-text>
			</button>
			<div v-if="!player.infoboxes[layer][data]" class="story-text" v-bind:style="tmp[layer].infoboxes[data].bodyStyle">
				<display-text :data="tmp[layer].infoboxes[data].body"></display-text>
			</div>
		</div>
		`,
  });
  ...
  Vue.component("challenge", {
    ...
    template: `
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(options.hideChallenges && maxedChallenge(layer, [data]) && !inChallenge(layer, [data]))"
			...
			<br><h3><display-text :data="tmp[layer].challenges[data].name"></display-text></h3><br><br>
			...
			<display-text v-if="tmp[layer].challenges[data].fullDisplay" :data="tmp[layer].challenges[data].fullDisplay"></display-text>
			<span v-else>
				<display-text :data="tmp[layer].challenges[data].challengeDescription"></display-text>
				Goal:  <display-text :data="tmp[layer].challenges[data].goalDesctiption"></display-text><br>
				Reward: <display-text :data="tmp[layer].challenges[data].rewardDisplay"></display-text><br>
				<span v-if="tmp[layer].challenges[data].rewardDisplay">Currently: <display-text :data="tmp[layer].challenges[data].rewardDisplay"></display-text></span>
			</span>
  ...
  Vue.component("mode", {
    props: ["layer", "data"],
    template: `
    <button @click="selectMode(data)" style="height: 70px;width: 70px" 
    v-bind:class="{
     tooltipBox: true,
     btn: true,
     ob: true,
     opt: !hasSelectedMode(tmp['options-tab'].modes[data].name),
     optSelected: hasSelectedMode(tmp['options-tab'].modes[data].name)
     }">
    <span>
    <display-text class="mode" :data="tmp['options-tab'].modes[data].name"></display-text>
    </span>
    <component class="tooltipBox tooltip" v-bind:is="{ template: '<div>' + modes[data].tooltip + '</div' }"/>
    </button>
    `,
  });
  ...
  Vue.component("modes", {
    props: ["layer"],
    template: `
    <div v-if="tmp['options-tab'].modes" class="upgTable">
     <div v-for="row in tmp['options-tab'].modes.rows" class="upgRow">
      <div v-for="col in tmp['options-tab'].modes.cols">
      <div v-if="tmp['options-tab'].modes[row*10+col]!== undefined" class="upgAlign">
       <mode :data="row*10+col"></mode>
      </div></div>
     </div>
     <br><br>Mode: {{calcModeName()}}
     <br>Balancing: {{calcModeBalance()}}
     <br><br><button @click="newSave()" style="color: black;height: 10px" class="btn opt ob">new save</button>
    </div>
    `,
  });
  ...
  Vue.component("upgrades", {
    props: ["layer", "data"],
    template: `
		<div id="modes" v-if="tmp[layer].upgrades" class="upgTable">
			<div v-for="row in (data === undefined ? tmp[layer].upgrades.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].upgrades.cols"><div v-if="tmp[layer].upgrades[row*10+col]!== undefined && 
        tmp[layer].upgrades[row*10+col].unlocked" class="upgAlign">
					<upgrade :layer = "layer" :data = "row*10+col" 
          v-bind:style="tmp[layer].componentStyles.upgrade"></upgrade>
				</div></div>
			</div>
			<br>
		</div>
  ...
  Vue.component("upgrade", {
    ...
    template: `
      ...
      <display-text v-if="tmp[layer].upgrades[data].fullDisplay" :data="tmp[layer].upgrades[data].fullDisplay"></display-text>
      <span v-else>
				<span v-if="tmp[layer].upgrades[data].title"><h3><display-text :data="tmp[layer].upgrades[data].title"></display-text></h3><br></span>
				<display-text :data="tmp[layer].upgrades[data].description"></display-text>
        <display-text v-if="tmp[layer].upgrades[data].effectDisplay" :data="'currently: ' + tmp[layer].upgrades[data].effectDisplay"></display-text>
        <span v-if="tmp[layer].upgrades[data].costDisplay"><display-text :data="'Cost: ' + tmp[layer].upgrades[data].costDisplay"></display-text></span>
				<span v-else><br><br>Cost: {{ formatWhole(tmp[layer].upgrades[data].cost) }} {{(tmp[layer].resource)}}</span>
			</span>
  ...
  Vue.component("milestone", {
    ...
    template: `
		<td v-if="tmp[layer].milestones && tmp[layer].milestones[data] !== undefined && milestoneShown(layer, data) && tmp[layer].milestones[data].unlocked" v-bind:style="[tmp[layer].milestones[data].style]" v-bind:class="{milestone: !hasMilestone(layer, data), tooltipBox: true, milestoneDone: hasMilestone(layer, data)}">
			<display-text :data="'<h3>' + tmp[layer].milestones[data].requirmentDescription + '</h3>'"></display-text><br>
			<display-text :data="tmp[layer].milestones[data].effectDescription"></display-text><br>
      <display-text v-if="tmp[layer].milestones[data].effectDisplay" :data="'<br>currently: ' + tmp[layer].milestones[data].effectDisplay"></display-text><br>
  ...
  Vue.component("prestige-button", {
    ...
    template: `
	  ...
	  @click="doReset(layer)">
    <display-text v-if="tmp[layer].prestigeButtonText" :data="tmp[layer].prestigeButtonText"></display-text>
    <span v-else v-html="prestigeButtonText(layer)"></span>
    </button>
		`,
  });
  ...
  // Displays the base resource for the layer, as well as the best, total, best in one reset, time since last reset and amount of resets done, data is amount of precision to use
  Vue.component("resource-display", {
    props: ["layer", "data"],
    template: `
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>You have {{format(tmp[layer].baseAmount, (data ? data : 0))}} {{tmp[layer].baseResource}}</span>
			<span v-if="tmp[layer].passiveGeneration"><br>You are gaining {{format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration), (data ? data : 0))}} {{tmp[layer].resource}} per second</span>
			<br><br>
			<span v-if="tmp[layer].showBest">Your best {{tmp[layer].resource}} is {{format(player[layer].best, (data ? data : 0))}}<br></span>
			<span v-if="tmp[layer].showTotal">You have made a total of {{format(player[layer].total, (data ? data: 0))}} {{tmp[layer].resource}}<br></span>
      <span v-if="tmp[layer].showResets">You have done {{formatWhole(player[layer].resets)}} {{tmp[layer].name}} resests<br></span>
      <span v-if="tmp[layer].showBestOnReset">Your best {{tmp[layer].resource}} gained in one reset is {{format(player[layer].bestOnReset, (data ? data: 0))}}<br></span>
      <span v-if="tmp[layer].showResetTime">you spent {{formatTime(player[layer].resetTime)}} in current reset<br><br></span>
		</div>
		`,
  });
  ...
  Vue.component("buyable", {
    ...
    template: `
		<div v-if="tmp[layer].buyables && tmp[layer].buyables[data]!== undefined && tmp[layer].buyables[data].unlocked" style="display: grid">
			...
			v-on:click="if(!interval) buyBuyable(layer, data)" :id='"buyable-" + layer + "-" + data' @tapstart="start" @tapend="stop" @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop">
				<display-text v-if="tmp[layer].buyables[data].title" :data="'<h2>' + tmp[layer].buyables[data].title + '</h2>'"></display-text><br>
				<display-text v-if="tmp[layer].buyables[data].display" v-bind:style="{'white-space': 'pre-line'}" :data="tmp[layer].buyables[data].display"></display-text>
  ...
  Vue.component("clickable", {
    ...
    template: `
		<button 
			...
			v-on:click="if(!interval) clickClickable(layer, data)" :id='"clickable-" + layer + "-" + data' @tapstart="start" tapend="stop" @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop">
			<display-text v-if="tmp[layer].clickables[data].title" :data="'<h2>' + tmp[layer].clickables[data].title + '</h2>'"></display-text><br>
			<display-text v-bind:style="{'white-space': 'pre-line'}" :data="tmp[layer].clickables[data].display"></display-text>
  ...
  Vue.component("master-button", {
    ...
    template: `
		<button v-if="tmp[layer].clickables && tmp[layer].clickables.masterButtonPress && !(tmp[layer].clickables.showMasterButton !== undefined && tmp[layer].clickables.showMasterButton == false)"
			v-on:click="run(layers[layer].clickables.masterButtonPress, layers[layer].clickables)" v-bind:class="{ longUpg: true, btn:true, can: player[layer].unlocked, locked: !player[layer].unlocked }"><display-text :data="tmp[layer].clickables.masterButtonText"></display-text></button>
	`,
  });
  ...
  Vue.component("gridable", {
    ...
    template: `
		...
		v-on:click="clickGrid(layer, data)" @tapstart="start" @tapend="stop" @mousedown="start" @mouseleave="stop" @mouseup="stop" @touchstart="start" @touchend="stop" @touchcancel="stop">
  ...
  Vue.component("microtabs", {
    ...
    template: `
		<div v-if="tmp[layer].microtabs">
			...
			<layer-tab style="margin-top: 100px" v-if="tmp[layer].microtabs[data][player.subtabs[layer][data]].embedLayer" :layer="tmp[layer].microtabs[data][player.subtabs[layer][data]].embedLayer" :embedded="true"></layer-tab>
			<column style="margin-top: 100px" v-else v-bind:style="tmp[layer].microtabs[data][player.subtabs[layer][data]].style" :layer="layer" :data="tmp[layer].microtabs[data][player.subtabs[layer][data]].content"></column>
		</div>
		`,
  });
    ...
    template: `
		<div v-if="tmp[layer].bars && tmp[layer].bars[data].unlocked" v-bind:style="{'position': 'relative'}"><div v-bind:style="[tmp[layer].bars[data].style, style.dims, {'display': 'table'}]">
			<div class="overlayTextContainer barBorder" v-bind:style="[tmp[layer].bars[data].borderStyle, style.dims]">
				<display-text class="overlayText" v-bind:style="[tmp[layer].bars[data].style, tmp[layer].bars[data].textStyle]" :data="tmp[layer].bars[data].display"></display-text>
			</div>
  ...
  Vue.component("achievement", {
    ...
    template: `
		...
		"></tooltip>
			<span v-if="tmp[layer].achievements[data].name"><br><display-text v-bind:style="tmp[layer].achievements[data].textStyle" :data="'<h3>' + tmp[layer].achievements[data].name + '<h3>'"></display-text><br></span>
		</div>
		`,
  });
  ...
  Vue.component("tree", {
    ...
    template: `<span id="treeStuff"><div class="blue" :style="{width: tmp.other.screenWidth}"><br><feature></feature><br><br><news></news></div>
    <table v-bind:class="{aTree: data[1]}">
  ...
  Vue.component("saves", {
    template: `
		<div class="savePopup">
			<div>
				<div class="upgRow" style="width: 100%" v-for="(data, name) in tmp['options-tab'].allSaves" 
        v-if="name!='set' && data!==undefined">
					<pre><div v-bind:style="{ color: tmp['options-tab'].allSaves.set==name ? 'magenta':'white'}">{{name}} </div></pre>
					<button 
            v-on:click="save(); loadSave(name);" style="color: black" class="savePopupBtn can">
            Load
          </button>
					<button 
            v-on:click="renameSave(name)" style="color: black" class="savePopupBtn can">
            Rename
          </button>
					<button 
            v-on:click="deleteSave(name)" style="color: black" class="savePopupBtn can">
            Delete
          </button>
          <button 
            style="color: black" class="savePopupBtnMini can" v-on:click="moveSave(name, -1)" 
            v-bind:style="{ visibility: showMoveSaveBtn(name, 'up')?'visible':'hidden'}">↑
          </button><button 
            style="color: black" class="savePopupBtnMini can" v-on:click="moveSave(name, 1)" 
            v-bind:style="{ visibility: showMoveSaveBtn(name, 'down')?'visible':'hidden'}">↓
          </button>
          <br><br><br>
          </div>
				<div 
          class="upgRow" style="width: 100%">
        </div>
			</div>
		</div>
	`,
  ...
  Vue.component("save-bank", {
    template: `
    <div>
      <div class="upgRow" style="width: 100%" 
      v-for="save in tmp['options-tab'].customSaves">
      <button style="color: black;height: 50px;width: 50px;font-size: 20px" 
      onclick="importSave(save.saveCode, true)">{{save.name}}</button>
      <br>
      </div>
    </div>
    `,
  ...
  Vue.component("feature", {
    ...
    computed: {
      current() {
        for (feature in tmp["info-tab"].features) {
          if (run(tmp["info-tab"].features[feature].unlocked, tmp["info-tab"].features[feature]) === false) return tmp["info-tab"].features[feature]
        }
      },
      },
    template: `
    <display-div v-if="typeof current === 'object'" :data="'<div class=feature>' + current.display + '<br>('+ format(run(current.progress, current)*100) + '%</div>'"></display-div>
    <div v-else class="feature">All features unlocked!</div>
    `
  })
  ...
  Vue.component("news", {
    data() { return {
      newsTickerText: `
       ...
       position: 'relative',
       bottom: '2px',
       right: tmp.newsTicker.right + 'vw',
       'transition-duration': tmp.newsTicker.transition + 's'
       }">
      `
    }},
    template: `<div v-if="options.news" id="news" class="news black"><display-text :data="newsTickerText + tmp.newsTicker.current + '</span>'"></display-text></div>`
  })
  
  Vue.component("display-div", {
    props: ["data"],
    template: `<component :is="{ template: '<div>' + data + '</div>'}"/>`
  })
  ...
  Vue.component("b-tooltip", systemComponents["b-tooltip"]);
...
var no 
setInterval(function() { no = document.getElementById("app")},50)
