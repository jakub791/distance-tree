function prestigeButtonText(layer) {
	if (tmp[layer].type === "normal") return `${player[layer].points.lt(1e3) ? "Reset for " : ""}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource} ${tmp[layer].resetGain.lt(100) && player[layer].points.lt(1e3) ? `<br><br>Next at ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : format(tmp[layer].nextAt))} ${tmp[layer].baseResource}` : ""}`
	if (tmp[layer].type === "static") return `Reset for <b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource}<br><br>${player[layer].points.lt(30) ? (tmp[layer].baseAmount.gte(tmp[layer].nextAt) && (tmp[layer].canBuyMax !== undefined) && tmp[layer].canBuyMax ? "Next:" : "Req:") : ""} ${formatWhole(tmp[layer].baseAmount)} / ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAtDisp) : format(tmp[layer].nextAtDisp))} ${tmp[layer].baseResource}		`
...
function constructNodeStyle(layer) {
	...
	if (tmp[layer].image) style.push({'background-image': 'url("' + tmp[layer].image + '")'})
	if (tmp[layer].boxShadowColor) style.push({'box-shadow': '3px 4px '+tmp[layer].boxShadowColor})
...
function updateTabFormat(layer) {
	...
	// Update microtabs
	for (let family in layers[layer].microtabs) {
