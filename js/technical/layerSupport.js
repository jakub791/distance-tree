...
function setupLayer(layer){
    ...
    if (layers[layer].buyables){
        ...
        for (let thing in layers[layer].buyables){
            if (isPlainObject(layers[layer].buyables[thing])){
                ...
                if (layers[layer].buyables[thing].canBuyMax === undefined) layers[layer].buyables[thing].canBuyMax = true
            }  
    ...
    if (layers[layer].startData) {
        ...
        if (data.showBest) layers[layer].showBest = true
        if (data.showTotal) layers[layer].showTotal = true
        if (data.showResets) layers[layer].showResets = true
        if (data.showBestOnReset) layers[layer].showBestOnReset = true
        if (data.showResetTime) layers[layer].showResetTime = true
    }
    ...
    if(layers[layer].softcap === undefined) layers[layer].softcap = new Decimal(Infinity)
	...
	if(layers[layer].color === undefined) layers[layer].color = "#4BDC13"
...
addLayer("options-tab", {
    tabFormat: [{name: "microtabs", data: "options"}],
  microtabs: {
    options: {
      main: {
        content: [{name: "options-tab"}]
      },
      saves: {
        content: [{name: "saves"}]
      },
      "save-bank": {
        content: [{name: "save-bank"}]
      },
      "modes and new save": {
        content: [{name: "modes"}]
      }
	...
	tooltip: "",
    row: 0,
  allSaves() { return allSaves },
  customSaves() { return customSaves },
  modes() { return modes },
})

addLayer("info-tab", {
    tabFormat: [{name: "info-tab"}],
	symbol: "Info",
	color: "#4BDC13",
	tooltip: "",
    row: 0,
    ...
    newsTickers() {
      return newsTickers.filter(isTickerUnlocked) 
    },
  newsTickerText: `
  nooo
  `
...
addLayer("changelog-tab", {
    tabFormat() {return ([{name: "display-text", data: modInfo.changelog}])},
	symbol: "Changelog",
	color: "#4BDC13",
	tooltip: "",
    row: 0
