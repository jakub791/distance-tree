...
function hardReset(force = false, resetOptions) {
    if (!force && !confirm("Are you sure you want to do this? You will lose all your progress on this save!")) return
...
function newSave(mode = tmp.selectedModes) {
    if (Object.keys(allSaves).length >= 100) { //too many saves will break things
      alert("You have too many saves, please delete an exsisting save!")
      return
    }
    let newName = prompt("Enter save name: ")
    newName = newName.replace(saveRegexCode, "") // Removes all non-alphanumeric characters
    if (newName === "set") {
      ...
      return
        ...
        let startPlayer = getStartPlayer()
...
function load() {
    ...
    if (options.offlineProd) {
        if (player.offTime === undefined) player.offTime = {
            remain: 0
        }
    ...
    versionCheck()
    if (inMode("absurd") && !confirm("Do you want to continue playing in absurd mode?")) {
      newSave()
      return
    }
    changeTheme()
...
function NaNcheck(data) {
    let curr = true
    for (item in data) {
        if (data[item] == null) {} else if (Array.isArray(data[item])) {
            curr = curr && NaNcheck(data[item])
        } else if (data[item] !== data[item] || checkDecimalNaN(data[item])) {
            if (!NaNalert) {
                clearInterval(interval)
                NaNalert = true
                alert("Invalid value found in player, named '" + item + "'. Please let the creator of this gane know! You can refresh the page, and you will be un-NaNed.")
                return false
            }
...
function exportSave() {
    ...
    doPopup("export", "", "Save Exported!", 3, "#25B9E3");
}
 
function importSave(imported = undefined, custom = false, forced = false) {
    if (custom && !confirm("are you sure you want to import this save?")) return
    if (!custom && imported === undefined) imported = prompt("Paste your save here")
    try {
...
function getStartLayerData(layer) {
    ...
    if (layerdata.points === undefined) layerdata.points = decimalZero
    ...
    if (layerdata.resets === undefined) layerdata.resets = decimalZero
    if (layerdata.bestOnReset === undefined) layerdata.bestOnReset = decimalZero
